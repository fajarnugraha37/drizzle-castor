...
import { companiesTable, usersTable, profilesTable, postsTable, commentsTable, groupsTable, userGroups } from "./schema";

export async function seed(db: BunSQLiteDatabase, args?: string[]) {
    ...
    if (groupsCount === 0) {
        for (let i = 1; i <= 5; i++) {
            await db.insert(groupsTable).values({
                name: faker.helpers.arrayElement(["Admin", "Editor", "Viewer", "Guest", "Moderator"]),
            });
        }
    }
    ...
    if (usersCount === 0) {
        for (let i = 1; i <= 25; i++) {
            ...
            const user = db.insert(usersTable).values({
                ...
                tags: faker.helpers.arrayElements(["admin", "editor", "viewer"], faker.number.int({ min: 1, max: 3 })),
                persona: {
                    hobbies: faker.helpers.arrayElements(["reading", "gaming", "traveling", "cooking"], faker.number.int({ min: 1, max: 4 })),
                    skills: faker.helpers.arrayElements(["JavaScript", "TypeScript", "Python", "SQL"], faker.number.int({ min: 1, max: 4 })),
                },
